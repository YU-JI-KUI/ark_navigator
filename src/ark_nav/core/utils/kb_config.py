...
class KBConfig:
    ...
    # 实现模式："local"（本地 FAISS）/ "remote"（远程 REST API）/ "none"（跳过检索，直连大模型）
    ...
    def check_required(cls):
        ...
        if cls.MODE.lower() not in ("local", "remote", "none"):
            raise ValueError(f"KB_MODE 取值必须为 'local' / 'remote' / 'none'，当前为: {cls.MODE!r}")
